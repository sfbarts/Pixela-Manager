...
def create_graph():
    ...
    try:
        ...
        graphs_dict[graph_config["name"]] = graph_config["id"]
...
def delete_graph():
    ...
    delete_endpoint = f"{graph_endpoint}/{selected_id}"
    try:
        response = requests.delete(url=delete_endpoint, headers=headers)
    except requests.exceptions.HTTPError:
        print("Could not delete graph, try again")
    else:
        graphs_dict.pop(graph_name.get())
        refresh_graphs_options()
        update_image("")
...
def add_pixel():
    ...
    selected_id = graphs_dict[graph_name.get()]
    add_pixel_endpoint = f"{graph_endpoint}/{selected_id}"
    pixel_config = {
        "date": date_selected,
...
def refresh_graphs_options():
    new_graphs_names = list(graphs_dict.keys())
